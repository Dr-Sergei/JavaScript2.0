...
function f8() {
  ...
  d8[d8.length] = d8[d8.length - 1];
  for (let i = d8.length - 2; i >= 0; i--) {
    d8[i + 1] = d8[i];
  }
  d8[0] = i8;
  // d8[d8.length] = d8[d8.length - 1];
  // d8[d8.length - 2] = d8[d8.length - 3];
  // d8[d8.length - 3] = d8[d8.length - 4];
  // d8[d8.length - 5] = d8[d8.length - 6];
  // d8[0] = i8;
  console.log(d8);
  showArr(".out-8", d8);
}
...
function f11() {
  ...
  let out11_1 = document.querySelector(".out-11");
  let check = d11.indexOf(i11);
  if (check === -1) {
    out11_1.innerHTML = i11;
  } else {
    out11_1.innerHTML = d11.indexOf(i11);
  }
}
...
function f12() {
  ...
  while (i < d12.length) {
    if (d12[i] === i12) {
      out12.innerHTML = "индекс эллемента = " + i;
      return out12.innerHTML;
    } else {
      out12.innerHTML = "элемента " + i12 + "  в массиве нет!";
    }
    i++;
  }
}
...
function f14() {
  ...
  console.log(d14);
  console.log("Длина массива равна: " + d14.length);
  showArr(".out-14", d14);
}
