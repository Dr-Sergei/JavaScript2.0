...
function f8() {
  ...
  let s81 = document.querySelectorAll(".s-81>option");
  let out81 = "";
  // console.log(s81);
  for (let key in s81) {
    // console.log(s81[key].value);
    if (s81[key].selected) {
      out81 = s81[key].value;
    }
  }

  let s82 = document.querySelectorAll(".s-82>option");
  let out82 = "";
  // console.log(s81);
  for (let key in s82) {
    // console.log(s81[key].value);
  ...
  const promise_1 = new Promise((resolve) => {
    fetch(URL + "/api/27/sr/read/" + out81, {
      headers: requestHeaders,
    }).then((response) => resolve(response.json()));
  ...
  const promise_2 = new Promise((resolve) => {
    fetch(URL + "/api/27/sr/read?race=" + out82, {
      method: "GET",
      headers: requestHeaders,
    }).then((response) => resolve(response.json()));
    // .then((data) => console.log(data));
  ...
  Promise.all([promise_1, promise_2]).then((data) => {
    ...
    for (let key in data) {
      ...
      img.src = URL + data[key].result.image;
