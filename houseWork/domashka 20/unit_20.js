...
function t2(event) {
  ...
  // console.log(event);
  document.querySelector(".out-2").innerHTML = "keyCode: " + event.keyCode;
}
...
function t4(event) {
  ...
  event.keyCode > 96 || (event.keyCode >= 48 && event.keyCode <= 57)
    ? (out4.innerHTML += event.key)
    : console.log(event.key + " -> is upperCase!");
...
function t6(event) {
  if (event.keyCode > 96) {
    a[event.key] = event.key;
    document.querySelector(".i-6").value = a[event.key];
  }
  return false;
}
...
function t9(event) {
  ...
  // console.log("key: " + event.key);
  if (event.keyCode === 40) {
    let x = document.querySelector(".out-9").innerHTML;
    x++;
    document.querySelector(".out-9").innerHTML = x;
  }
}
...
function t11(event) {
  ...
  let arrBtn = document.querySelectorAll(".keyboard");
  ...
  for (item of arrBtn) {
    ...
    if (
      event.key == item.getAttribute("data") ||
      event.code.toLowerCase() == item.getAttribute("data")
    ) {
