...
function f9() {
  ...
  if (out !== "") {
    ...
    document.querySelector(".out-9").innerHTML = "___";
  }
}
...
function f17() {
  let out = "";
  for (let key in a17) {
    for (let elmt in a17[key]) {
      if (elmt === "name" && a17[key].age > 30) {
        out += a17[key][elmt] + " ";
      }
    }
  }
...
function f19() {
  ...
  if ((out.style.backgroundColor = true)) {
    out.innerHTML = "...";
    out.style.backgroundColor = "";
  }
  for (let key in a19) {
    for (let i = 0; i < a19[key].length; i++) {
      if (i19 === a19[key][i].toLowerCase() || i19 === a19[key][i]) {
        out.style.backgroundColor = key;
        out.innerHTML = key;
        // console.log(out.style.backgroundColor);
        // console.log(key);
        break;
      }
    }
